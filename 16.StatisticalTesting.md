...
> 본 노트의 모든 인용은 실존 문헌이며 말미에 DOI/URL을 단다. 본문 예시는 다기관 유방 MRI 기반 BRCA 변이 예측(기관 A=학습·내부검증, 기관 B=외부검증)을 예시 사례로 든다.

---

## 목차

★. [먼저 — 결정 트리와 숫자로 따라가는 검정 선택](#먼저--결정-트리와-숫자로-따라가는-검정-선택)
...
## 먼저 — 결정 트리와 숫자로 따라가는 검정 선택

> **이 노트의 결론을 한 장으로.** 어떤 검정을 쓸지는 결국 **네 개의 질문**으로 정해진다 — ① 변수가 연속형인가 범주형인가 → ② (연속형이면) 정규분포인가 → ③ 군이 몇 개이고 짝지어졌는가 → ④ (범주형이면) 기대도수가 충분한가/대응인가. 아래 트리로 길을 고르고, 이어지는 **숫자 예제**로 그 검정이 실제로 어떻게 계산되는지 손으로 따라간다. 각 항목은 뒤의 상세 섹션(가정·수식·문헌)으로 링크된다. 아직 용어가 낯설면 [§0 용어 정리](#0-용어-먼저--헷갈리는-개념부터-정리)를 먼저 봐도 좋다.

### ★-1. 큰 그림 — 네 개의 질문으로 내려가는 트리
...
├─① 변수 유형은?  ── 연속형 → ②   /   범주형 → ④
│
├─② [연속형] 정규분포인가?   (Shapiro–Wilk, Q–Q plot → §3-1)
│     ├─ 정규 O → ③ 로 (모수적 경로)      요약통계: mean ± SD
│     └─ 정규 X → 비모수 경로             요약통계: median [Q1, Q3]
│
├─③ [연속·정규] 군의 수와 짝지음(대응)?
│     ├─ 2군·독립 ─ 등분산 O → Student t      (§5-1)
│     │            └ 등분산 X → Welch t       (§5-2, 권장 기본값)
│     ├─ 2군·대응 ───────────────→ Paired t          (§5-4)
│     ├─ ≥3군·독립 ──────────────→ One-way ANOVA (+Tukey)   (§6-1)
│     └─ ≥3군·대응 ──────────────→ RM-ANOVA / Friedman
│
│   └─ [연속·비정규] 같은 자리에서 비모수로 치환:
│        2군·독립 → Mann–Whitney U          (§5-3)
│        2군·대응 → Wilcoxon signed-rank     (§5-4)
│        ≥3군·독립 → Kruskal–Wallis (+Dunn)  (§6-2)
│        ≥3군·대응 → Friedman
│
└─④ [범주형] 분할표를 만들고 → 기대도수/대응 확인  (§7-0)
      ├─ 독립·기대도수 모두 ≥5 → Pearson χ² (카이제곱)   (§7-1)
      ├─ 독립·기대도수 <5 칸 존재 → Fisher exact          (§7-2)
      ├─ 대응(같은 대상의 2값 측정) → McNemar             (§7-3)
      └─ 순서형 노출·용량–반응 추세 → Cochran–Armitage    (§7-4)
```

> **이 노트에서 가장 중요한 한 줄:** 요약통계와 검정은 **반드시 짝을 맞춘다.** `mean ± SD` ↔ 모수 검정(t/ANOVA/Pearson), `median [Q1, Q3]` ↔ 비모수 검정(Mann–Whitney/Kruskal–Wallis/Spearman). median으로 보고하면서 t-검정을 쓰면 그 자체로 불일치다. 표 전체를 통일하고 싶으면 — lab 수치(CRP·ALP·AST 등)는 대개 오른쪽으로 치우치므로 — **전부 `median [Q1, Q3]` + 비모수**로 가는 것이 가장 안전하다. (상세 결정표는 [§4](#4-의사결정-표--어떤-상황에-어떤-검정인가).)

### ★-2. 숫자로 따라가기 — 경로별 손계산 예제

> 트리의 각 잎(leaf)을 **실제 수치**로 한 번씩 통과한다. 예시는 다기관 유방 MRI BRCA 연구(기관 A·B)를 가정한다. 모든 임계값은 양측 $\alpha=0.05$ 기준.

#### (A) 연속·정규·2군·등분산 → **Student t** (예: 두 기관의 평균 나이)

| | $n$ | 평균 $\bar x$ | 표준편차 $s$ | 분산 $s^2$ |
|---|---|---|---|---|
| 기관 A | 10 | 52.0세 | 8.0 | 64 |
| 기관 B | 12 | 47.0세 | 9.0 | 81 |

1. 합동분산(pooled): $s_p^2 = \dfrac{(10-1)\cdot64 + (12-1)\cdot81}{10+12-2} = \dfrac{576+891}{20} = 73.35$, 따라서 $s_p = 8.56$.
2. 검정통계량: $t = \dfrac{52.0-47.0}{8.56\sqrt{\tfrac{1}{10}+\tfrac{1}{12}}} = \dfrac{5.0}{8.56\times0.428} = \dfrac{5.0}{3.67} = 1.36$.
3. 자유도 $df = 10+12-2 = 20$, 임계값 $t_{0.975,20}=2.086$.
4. $1.36 < 2.086$ → **유의하지 않음** ($p \approx 0.19$). 5세 차이지만 표본이 작아 우연으로 설명 가능.

> 교훈: "평균 차이가 5세나 되는데?"라도 분모(불확실성)에 비하면 작다 — $t$는 **차이 ÷ 불확실성**이다([§5](#5-두-군-비교--연속형)).

#### (B) 연속·정규·2군·이분산 → **Welch t** (예: 표본수·분산이 크게 다른 라디오믹스 entropy)
...
Levene 검정에서 분산이 다르다고 나왔다($p\le0.05$) → 등분산 가정을 버린다.

1. $t = \dfrac{5.0-6.5}{\sqrt{\tfrac{4}{8}+\tfrac{16}{20}}} = \dfrac{-1.5}{\sqrt{0.5+0.8}} = \dfrac{-1.5}{1.14} = -1.32$.
2. Welch–Satterthwaite 자유도:

$$
df \approx \frac{(0.5+0.8)^2}{\tfrac{0.5^2}{8-1}+\tfrac{0.8^2}{20-1}} = \frac{1.69}{0.0357+0.0337} = \frac{1.69}{0.0694} = 24.4
$$

3. $|{-1.32}| < t_{0.975,24}=2.064$ → **유의하지 않음** ($p \approx 0.20$).

> 교훈: 분산·표본수가 다르면 Student t는 1종 오류율이 틀어진다. 현대 권장은 "**그냥 처음부터 Welch를 기본값**"([§5-2](#5-2-welch-t-검정-이분산-권장-기본값), Delacre 2017).

#### (C) 연속·비정규·2군 → **Mann–Whitney U** (예: 치우친 종양 크기, cm)

- 군 A($n_1=5$): 1.2, 1.5, 2.0, 2.3, **8.0** ← 이상치
- 군 B($n_2=4$): 1.0, 1.1, 1.3, 1.4

이상치 때문에 평균·SD는 왜곡된다 → 순위 기반.

1. 전체를 오름차순 정렬하고 순위 부여:

| 값 | 1.0 | 1.1 | 1.2 | 1.3 | 1.4 | 1.5 | 2.0 | 2.3 | 8.0 |
|---|---|---|---|---|---|---|---|---|---|
| 군 | B | B | A | B | B | A | A | A | A |
| 순위 | 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8 | 9 |

2. 순위합: $R_A = 3+6+7+8+9 = 33$, $R_B = 1+2+4+5 = 12$ (합 45 = $\tfrac{9\cdot10}{2}$ ✓).
3. $U_A = R_A - \tfrac{n_1(n_1+1)}{2} = 33 - 15 = 18$, $U_B = 12 - 10 = 2$ (합 $=n_1 n_2 = 20$ ✓). $U=\min=2$.
4. $n_1=5, n_2=4$에서 임계값은 $U \le 1$이라야 기각 → $U=2$이므로 **유의하지 않음** ($p>0.05$).
5. 덤: $\text{AUC} = U_A/(n_1 n_2) = 18/20 = 0.90$ — "A가 B보다 크다"의 판별력이 0.90인데도 표본이 작아 유의하지 않다.

> 교훈: median A=2.0 vs B=1.2로 차이가 뚜렷하고 AUC=0.90이어도 **$n$이 작으면 검정력이 없다**. Mann–Whitney U는 ROC-AUC와 수학적으로 동치다([§5-3](#5-3-mannwhitney-u-비모수), [§12-1](#12-1-판별력discrimination-roc-auc)).

#### (D) 연속·정규·≥3군 → **One-way ANOVA** (예: 분자아형별 종양 크기)

| 아형 | 값 | $\bar x_j$ |
|---|---|---|
| Luminal ($n=4$) | 2, 3, 4, 3 | 3.0 |
| HER2 ($n=4$) | 4, 5, 6, 5 | 5.0 |
| TNBC ($n=4$) | 6, 7, 8, 7 | 7.0 |

전체 평균 $\bar x = 5.0$.

1. 군간변동: $SS_\text{between} = \sum n_j(\bar x_j-\bar x)^2 = 4(3-5)^2+4(5-5)^2+4(7-5)^2 = 16+0+16 = 32$, $df=k-1=2$, $MS_\text{between}=16$.
2. 군내변동: 각 군의 편차제곱합이 모두 $(-1)^2+0+1^2+0=2$ → $SS_\text{within}=6$, $df=N-k=9$, $MS_\text{within}=0.667$.
3. $F = \dfrac{MS_\text{between}}{MS_\text{within}} = \dfrac{16}{0.667} = 24.0$, $df=(2,9)$, 임계값 $F_{0.95}(2,9)=4.26$.
4. $24.0 \gg 4.26$ → **유의함** ($p<0.001$). 이어서 어느 쌍이 다른지 **Tukey HSD** 사후검정([§6-3](#6-3-사후검정post-hoc--다중성-통제-필수)).

> 교훈: 쌍별 t-검정 3번 대신 **전반검정(omnibus) 한 번 → 유의하면 사후검정**. $F$는 "군간 차이 ÷ 군내 잡음"이다([§6](#6-세-군-이상-비교와-사후검정)).

#### (E) 연속·비정규·≥3군 → **Kruskal–Wallis** (예: 치우친 바이오마커, 군당 3명)

군 G1: 1,2,3 / G2: 4,5,6 / G3: 7,8,9 → 순위합 $R_1=6, R_2=15, R_3=24$, $N=9$.

$$
H = \frac{12}{N(N+1)}\sum_j\frac{R_j^2}{n_j} - 3(N+1) = \frac{12}{90}\Big(\frac{36}{3}+\frac{225}{3}+\frac{576}{3}\Big) - 30 = 0.1333\times279 - 30 = 7.2
$$

$H \sim \chi^2_{k-1}=\chi^2_2$, 임계값 5.99 → $7.2 > 5.99$, **유의함** ($p\approx0.027$). 사후는 Dunn 검정+보정([§6-2](#6-2-kruskalwallis-비모수)).

#### (F) 범주형·독립·기대도수 충분 → **Pearson χ²** (예: 기관 vs BRCA 양성)
...
1. 기대도수 $E_{ij}=\tfrac{(\text{행합})(\text{열합})}{N}$: BRCA+ 칸 $=\tfrac{40\times100}{200}=20$ (양 기관), BRCA− 칸 $=\tfrac{160\times100}{200}=80$. **모두 ≥5 → χ² 사용 가능**.
2. $\chi^2 = \dfrac{(30-20)^2}{20}+\dfrac{(10-20)^2}{20}+\dfrac{(70-80)^2}{80}+\dfrac{(90-80)^2}{80} = 5+5+1.25+1.25 = 12.5$.
3. $df=(2-1)(2-1)=1$, 임계값 3.84 → $12.5 > 3.84$, **유의함** ($p\approx0.0004$).
4. 효과크기 OR $=\dfrac{ad}{bc}=\dfrac{30\times90}{10\times70}=3.86$ — 기관 A에서 BRCA+ 오즈가 3.86배.

> 교훈: 카이제곱의 분기 기준은 **관측도수가 아니라 기대도수**다(Cochran's rule, [§7-1](#7-1-pearson-χ카이제곱-검정--독립성적합도)).

#### (G) 범주형·독립·기대도수 부족 → **Fisher exact** (예: 희귀 변이, 소표본)
...
1. 기대도수 점검: 변이+ / 기관 B 칸 $=\tfrac{5\times20}{40}=2.5 < 5$ → **χ² 무효 → Fisher**.
2. 주변합을 고정하고 초기하분포로 정확확률 계산. 관측표(변이 5건이 모두 기관 A)의 확률:

$$
P = \frac{\binom{5}{5}\binom{35}{15}}{\binom{40}{20}} = \frac{1\times 3{,}247{,}943{,}160}{137{,}846{,}528{,}820} = 0.0236
$$

3. $b=0$이라 한쪽으로 더 극단적인 표가 없다 → 일측 $p = 0.024$ ($\le0.05$, **유의**). 양측은 반대 극단 표까지 더해 $\approx0.05$ 부근.

> 교훈: 0이나 한 자릿수 칸이 있는 2×2는 χ²의 근사가 깨진다 — **정확검정(Fisher)** 으로 간다([§7-2](#7-2-fisher-exact)).

#### (H) 범주형·대응 → **McNemar** (예: 두 판독기의 양성 여부, 같은 환자 100명)
...
1. 일치 칸(40, 40)은 무시하고 **불일치 칸만** 사용: $b=15, c=5$.
2. $\chi^2 = \dfrac{(b-c)^2}{b+c} = \dfrac{(15-5)^2}{15+5} = \dfrac{100}{20} = 5.0$, $df=1$, 임계값 3.84.
3. $5.0 > 3.84$ → **유의함** ($p\approx0.025$): 두 판독기의 양성률이 다르다.

> 교훈: 같은 대상을 두 번 측정한 자료(전/후, 두 판독기, 두 모델의 분류)는 일반 χ²가 아니라 **McNemar**. 연속형의 대응 버전은 Paired t / Wilcoxon signed-rank다([§5-4](#5-4-대응표본), [§7-3](#7-3-mcnemar-대응-2값)).

#### (I) 연속·대응 → **Paired t** (예: 같은 종양을 두 번 잰 차이 $d$, 6명)

차이 $d$: 0.2, −0.1, 0.3, 0.0, 0.4, 0.2 → $\bar d = 1.0/6 = 0.167$, $s_d = 0.186$.

$$
t = \frac{\bar d}{s_d/\sqrt{n}} = \frac{0.167}{0.186/\sqrt{6}} = \frac{0.167}{0.0760} = 2.19,\quad df=5
$$

임계값 $t_{0.975,5}=2.571 > 2.19$ → **유의하지 않음** ($p\approx0.08$, 경계). 대응 자료는 **차이 $d$ 하나로 축소**해 0과 비교하는 것이 핵심 — 환자 간 변동이 상쇄돼 검정력이 올라간다.

---

이 트리와 9개 예제가 노트 전체의 뼈대다. 이제부터는 각 단계를 **왜 그렇게 정하는지(가정)** 와 **수식의 의미**를 차례로 깊게 본다.
...
예시 사례에서는 패러다임 간(예: CL+RA RF vs Image) AUC 비교에 DeLong을 쓴다. 독립 표본(외부 코호트)이면 비상관 분산을, 짝지어진(같은 환자) 경우 공분산을 포함한다.
...
- **K-fold CV**: 데이터를 $K$등분, 매번 1 fold 검증. 분류는 **stratified** K-fold로 클래스 비율 유지. 소표본 의료 데이터에서 단일 split은 분산이 커, **반복(repeated) CV**나 다중 seed로 안정성을 본다(예: 5-seed/10-split 반복).
