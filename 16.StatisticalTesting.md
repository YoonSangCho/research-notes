...
| **명목형(2값)** | ER 양성/음성, BRCA+/− | n (%) | χ²/Fisher, McNemar(대응) |
| **명목형(다값)** | 분자아형(Luminal/HER2/TNBC) | n (%) | χ² |
| **생존(time-to-event)** | 무재발생존 | KM 곡선, median OS | log-rank, Cox |

치우친 연속형을 평균±SD로 보고하면 분포를 왜곡한다. 우리 Table 1 코드는 **변수마다 정규성을 먼저 검정**해 정규면 `mean ± SD`, 비정규면 `median [Q1, Q3]`로 자동 분기한다 — 이것이 정석이다.

---

## 3. 검정 전 가정 점검

모수 검정(t, ANOVA, Pearson)은 가정이 깨지면 1종 오류율이 명목 $\alpha$를 벗어난다. 검정 *전에* 가정을 점검한다.

### 3-1. 정규성(normality)

- **Shapiro–Wilk 검정** *(Shapiro & Wilk 1965)* — 표본 정규성 검정의 표준. 통계량
$$
W = \frac{\left(\sum_{i=1}^{n} a_i x_{(i)}\right)^2}{\sum_{i=1}^{n}(x_i - \bar{x})^2}
$$
$x_{(i)}$는 정렬된 순서통계량, $a_i$는 정규분포 순서통계량의 기대·공분산에서 유도된 상수. $W$가 1에 가까울수록 정규. 작은 표본($n<50$)에서 검정력이 우수하다.
- **Kolmogorov–Smirnov / Lilliefors, Anderson–Darling**: 대표본 보조 수단.
- **Q–Q plot**: 검정의 p-value보다 시각적 진단이 신뢰성 있다. **대표본에서는 미세한 비정규도 유의해지므로** Shapiro의 p값만 맹신하지 말 것.
...
- **Levene 검정** *(Levene 1960)* — 평균으로부터의 절대편차 $z_{ij}=|x_{ij}-\bar{x}_j|$에 일원배치 ANOVA를 적용. 중앙값 기반(Brown–Forsythe) 변형이 비정규에 강건.
- 등분산이 깨지면 **Welch 보정**(t/ANOVA)으로 간다. 현대 통계 관행은 "굳이 등분산을 가정하지 말고 처음부터 Welch를 기본값으로"를 권장한다 *(Delacre et al. 2017)*.

### 3-3. 독립성

관측치 독립 가정. **같은 환자의 양측 유방·반복 측정·군집 표본**은 위반 → 혼합효과모형(mixed model)/GEE, 또는 대응표본 검정(paired/McNemar)으로 처리. 다기관 데이터에서 *기관*은 군집 변수다.
...
| 사후(비모수) | — | **Dunn**(+보정) | — |
| 2값×2값 빈도 | 기대도수 모두 ≥5 | **Pearson χ²** | **McNemar** |
...
> ⚠️ "**기대도수 <5 → Fisher**"는 *expected* count 기준이지 관측도수가 아니다(Cochran's rule). 우리 Table 1 코드는 2×2의 기대도수를 계산해 모두 ≥5면 χ², 하나라도 <5면 Fisher로 자동 분기한다.
...
두 독립 표본 $n_1, n_2$, 표본평균 $\bar{x}_1,\bar{x}_2$. 합동분산(pooled variance)
...
### 7-1. Pearson χ² (독립성/적합도)

$r\times c$ 분할표에서 관측도수 $O_{ij}$, 기대도수 $E_{ij}=\tfrac{(\text{행합})(\text{열합})}{N}$ *(Pearson 1900)*:

$$
\chi^2 = \sum_{i,j}\frac{(O_{ij}-E_{ij})^2}{E_{ij}}, \quad df=(r-1)(c-1)
$$

**유효성 조건**: 기대도수의 80% 이상이 ≥5, 모든 칸 ≥1(Cochran's rule). 2×2에서는 Yates 연속성 보정이 논쟁적 — 표본이 작으면 보정 대신 Fisher가 낫다.
...
우리 연구는 패러다임 간(예: CL+RA RF vs Image) AUC 비교에 DeLong을 쓴다(`_summary/delong_*.csv`). 독립 표본(외부 코호트)이면 비상관 분산을, 짝지어진 경우 공분산을 포함한다.
